...
@router.message(RoleFilter("technician"), F.text.in_(["🌐 Tilni o'zgartirish", "🌐 Изменить язык"]))
...
@router.callback_query(RoleFilter("technician"), F.data.startswith("lang_"))
async def language_callback_handler(callback: CallbackQuery):
    ...
    if success:
        ...
        keyboard = get_technician_main_menu(language)
