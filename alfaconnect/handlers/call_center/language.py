...
@router.message(RoleFilter("callcenter_operator"), F.text.in_( ["🌐 Tilni o'zgartirish", "🌐 Изменить язык"]))
...
@router.callback_query(RoleFilter("callcenter_operator"), F.data.startswith("lang_"))
async def language_callback_handler(callback: CallbackQuery):
    ...
    if success:
        ...
        keyboard = get_call_center_main_keyboard(language, callback.from_user.id)
