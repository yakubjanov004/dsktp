...
@router.message(RoleFilter("controller"), F.text.in_(["🌐 Tilni o'zgartirish", "🌐 Изменить язык"]))
...
@router.callback_query(RoleFilter("controller"), F.data.startswith("lang_"))
async def language_callback_handler(callback: CallbackQuery):
    ...
    if success:
        ...
        keyboard = get_controller_main_menu(language)
