...
@router.message(RoleFilter("callcenter_supervisor"), F.text.in_(["🌐 Tilni o'zgartirish", "🌐 Изменить язык"]))
...
@router.callback_query(RoleFilter("callcenter_supervisor"), F.data.startswith("lang_"))
async def language_callback_handler(callback: CallbackQuery):
    ...
    if success:
        ...
        keyboard = get_call_center_supervisor_main_menu(language, callback.from_user.id)
